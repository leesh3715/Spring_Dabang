				...
				<table class="is-table-hor">
					...
					<c:forEach items="${list }" var="dto">
						<tr>
							...
							<a href="board_view.do?b_no=${dto.getB_no() }&b_head=${head}">
									${dto.getB_title() }</a>
					...
					<c:if test="${empty list }">
							...
							<td colspan="4">
								...
								<c:if test="${nick eq admincheck}">	
									...
									<c:if test="${!empty find_field }">
										<a  class="is-btn-brown" onclick="deleteAction(${find_name},${page},'${head }')" style="color: white;">삭제</a>
									</c:if>
									<c:if test="${empty find_field }">
										<a  class="is-btn-brown" onclick="deleteAction('${head }')" style="color: white;">삭제</a>
									</c:if>
								</c:if>
								<c:if test="${!empty nick }">
									<a href="<%=request.getContextPath()%>/board_write.do" class="is-btn-brown">게시글 작성</a>
						...
						<c:if test="${page > block }"> <!-- 페이지가 블럭 3 보다 클 경우  이전으로 가는 버튼 생성 -->
							<c:if test="${empty find_name }">
								<a href="board_list.do?page=1&b_head=${head}">[◀◀]</a> <!-- 무조건 첫번째 페이지로 보내 줌  -->
								<a href="board_list.do?page=${startBlock - 1 }&b_head=${head}">[◀]</a> <!-- 현재 페이지의 첫번째 블럭에 -1을 해주어 이전 페이지의 마지막 블럭으로 이동  -->
							</c:if>
							<c:if test="${!empty find_name }">
						...
						<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
							...
							<c:if test="${!(i == page )}"> <!-- 현재 페이지랑 i가 다를 때는 연결할수 있게 링크를 걸어줌, i 에 해당하는 페이지 변수를 받아서  -->
								<c:if test="${empty find_name }">
									<a href="board_list.do?page=${i }&b_head=${head}">[${i }]</a>
								</c:if>
								<c:if test="${!empty find_name }">
						...
						<c:if test="${endBlock < allPage }"> <!-- 전체 페이지가 마지막 블럭보다 클 경우 다음으로 가는 버튼 생성 -->
							<c:if test="${empty find_name }">
								<a href="board_list.do?page=${endBlock + 1 }&b_head=${head}">[▶]</a> <!-- 현재 페이지의 마지막 블럭에 +1을 해주어 다음 페이지의 첫번째 블럭으로 이동  -->
								<a href="board_list.do?page=${allPage }&b_head=${head}">[▶▶]</a> <!-- 마지막 페이지의 끝으로 이동  -->
							</c:if>
							<c:if test="${!empty find_name }">
