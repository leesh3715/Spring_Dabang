		...
		<div class="is-frame-md">
			...
			<c:set var="admin" value="admin"></c:set>
				<div align="right" style="margin: 0px 10px 10px; padding:0px 10px 10px;"> 
					<c:if test="${nick==admin }">
						<input type="button" value="글쓰기" onclick="location.href='board_write.do?b_head=${dto.getB_head() }'">  
					</c:if>
					<c:if test="${dto.getB_writer()==nick }">
						<input type="button" value="수정" onclick="location.href='board_write.do?b_no=${dto.getB_no()}&b_head=${dto.getB_head() }'"> 
					</c:if>
					<c:if test="${dto.getB_writer()==nick || nick==admin}">
						<input type="button" value="삭제" onclick="deleteShere(${dto.getB_no() },'${head }')"> 
					</c:if>
					<input type="button" value="목록" onclick="location.href='notice_room.do?b_head=${dto.getB_head() }'"> 
				</div>
				...
				<table class="is-table-hor">
					...
					<tr>
						<c:if test="${!empty pre }">
							<td>
								<a href="board_view.do?b_no=${pre.getB_no() }&b_head=${head}"><font>이전글</font></a>
							</td>
							<td>
							<div align="left">
								<c:if test="${pre.getB_check()==1 }"><font color="red">[원글이 삭제된 답글]</font></c:if>
								<a href="board_view.do?b_no=${pre.getB_no() }&b_head=${head}">${pre.getB_title() }</a>
								...
								<c:if test="${pre.getB_Comments_count() !=0}">		
									&nbsp;
									<a href="board_view.do?b_no=${pre.getB_no() }&b_head=${head}">
						...
						<div style="background-color: #cccccc;">
							...
							<td>
								...
								<a href="board_view.do?b_no=${groupdto.getB_no() }&b_head=${head}">${groupdto.getB_title() }</a>
								<c:if test="${groupdto.getB_src() != null }">
									<img alt="" src="resources/images/img.jpg" width="25" height="25" style="opacity: 0.5;">
								</c:if>		
								<c:if test="${groupdto.getB_Comments_count() !=0}">		
									&nbsp;
									<a href="board_view.do?b_no=${groupdto.getB_no() }&b_head=${head}">
						...
						<c:if test="${!empty next }">
							<td>
								<a href="board_view.do?b_no=${next.getB_no() }&b_head=${head}"><font>다음글</font></a>
							</td>
							<td>
							<div align="left">
								<c:if test="${next.getB_check()==1 }"><font color="red">[원글이 삭제된 답글]</font></c:if>
								<a href="board_view.do?b_no=${next.getB_no() }&b_head=${head}">${next.getB_title() }</a>
								<c:if test="${next.getB_src() != null }">
									<img alt="" src="resources/images/img.jpg" width="25" height="25" style="opacity: 0.5;">
								</c:if>		
								<c:if test="${next.getB_Comments_count() !=0}">		
									&nbsp;
									<a href="board_view.do?b_no=${next.getB_no() }&b_head=${head}">
